...
# Buy 시그널 (초록색 점) - Combined 선 위로 이동
plt.scatter(buy_dates, df.loc[buy_dates, 'combined'], color='green', label='Buy Signal', marker='o', s=100)

# Sell 시그널 (빨간색 점) - Combined 선 위로 이동
plt.scatter(sell_dates, df.loc[sell_dates, 'combined'], color='red', label='Sell Signal', marker='o', s=100)
