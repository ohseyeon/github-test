...
# Buy 시그널 (초록색 점)
plt.scatter(buy_dates, df.loc[buy_dates, 'price_difference'], color='green', label='Buy Signal', marker='o', s=100)

# Sell 시그널 (빨간색 점)
plt.scatter(sell_dates, df.loc[sell_dates, 'price_difference'], color='red', label='Sell Signal', marker='o', s=100)

# USD/KRW 가격을 기준으로 원점 선 (0 차이선)
plt.axhline(0, color='red', linestyle='--', label='0 Difference Line')

# 그래프 설정
plt.title('Buy/Sell Signals Based on Price Difference and Adjusted USD/KRW')
plt.xlabel('Date')
